...
class _ServiceReportFormPageState extends State<ServiceReportFormPage> {
  ...
  static final Map<String, List<String>> _cachedPartnersByCompany = {};
  static final Map<String, List<String>> _cachedTechniciansByCompany = {};
  ...
  Future<void> _loadPartners() async {
    ...
    final cached = _cachedPartnersByCompany[companyId];
    if (cached != null) {
      setState(() {
        partnerList = cached;
        _isLoadingPartners = false;
    ...
    try {
      final snapshot = await CompanyCollectionResolver.partners()
          .orderBy('name')
          .get();
      ...
      _cachedPartnersByCompany[companyId] = data;
  ...
  Future<void> _loadTechnicians() async {
    try {
      ...
      final cached = _cachedTechniciansByCompany[companyId];
      if (cached != null) {
        setState(() {
          technicianList = cached;
      ...
      _cachedTechniciansByCompany[companyId] = data;
      ...
      setState(() {
        ...
        if (data['spareParts'] != null) {
          ...
          spareParts = rawSpareParts
              .map(
                (e) => ServiceReportSparePart.fromMap(
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.photo, color: Colors.teal, size: 18),
              ),
              const SizedBox(width: 10),
              const Text(
                'Media Summary',
            ...
            child: Row(
              ...
              children: [
                _mediaCountItem(
                  Icons.photo,
                  'Photos',
                  mediaCount > 0 ? mediaCount - (_video != null ? 1 : 0) : 0,
                ),
                _mediaCountItem(
                  Icons.videocam,
                  'Videos',
                  _video != null || _videoUrl != null ? 1 : 0,
                ),
              ],
            ),
          ),
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [Text(part.name), Text('Qty: ${part.qty}')],
                      ),
                    ),
                    IconButton(
  ...
  Widget _buildSignaturePreview() {
    if (_signature != null) {
      if (kIsWeb) {
        ...
        return Image.file(File(_signature!.path), fit: BoxFit.contain);
      }
      ...
      return Image.network(
        ...
        errorBuilder: (_, __, ___) =>
            const Center(child: Text("Failed to load signature")),
      );
    } else {
      return const Center(
        child: Column(
  ...
  Future<void> _submitReport() async {
    ...
    try {
      ...
      if (_currentReportId == null) {
        final newDocId = await ServiceReportFirestore.createServiceReport(
          data: {...reportData, "status": "Submitted"},
        );
