...
class OvernightService {
  Future<void> addOvernight({
    ...
    await CompanyFirestore.collection(
      'attendance',
    ).doc(employeeId).collection('overnight').add(entry.toFirestore());
  }
  ...
  Future<void> updateOvernight({
    ...
    await CompanyFirestore.collection(
      'attendance',
    ).doc(employeeId).collection('overnight').doc(docId).update({
      'period': entry.period,
  ...
  Future<void> deleteOvernight({
    required String employeeId,
    required String docId,
  }) async {
    await CompanyFirestore.collection(
      'attendance',
    ).doc(employeeId).collection('overnight').doc(docId).delete();
  }
}
