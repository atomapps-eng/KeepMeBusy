...
Future<void> _deleteOrderIn(
  ...
  await firestore.runTransaction(
    ...
    for (final item in items) {
      ...
      partSnaps[item['partId']] =
          await tx.get(partRef);
    }
    ...
    for (final item in items) {
      ...
      final currentStock =
          (partSnap['currentStock']
                  as num)
              .toInt();

      // 🔥 Order In → stock DIKURANGI
      tx.update(
        partSnap.reference,
        {
          'currentStock':
  ...
  ScaffoldMessenger.of(context)
    ...
    const SnackBar(
      content:
          Text('Order In berhasil dihapus'),
      backgroundColor:
          Colors.redAccent,
    ),
  );
